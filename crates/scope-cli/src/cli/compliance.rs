...
use scope::display::{OutputFormat, format_risk_report};
...
pub async fn handle_compliance_report(args: ComplianceReportArgs) -> anyhow::Result<()> {
    ...
    let mut pattern_results: Vec<(
        String,
        String,
        Option<scope::compliance::datasource::PatternAnalysis>,
    )> = Vec::new();
    ...
    for (addr, chain) in &addresses {
        ...
        let pat = if let Some(ref c) = client {
            c.get_transactions(addr, chain)
                .await
                .ok()
                .map(|txs| scope::compliance::datasource::analyze_patterns(&txs))
        } else {
            None
        };
...
fn format_compliance_report(
    assessments: &[scope::compliance::risk::RiskAssessment],
    patterns: &[(
        String,
        String,
        Option<scope::compliance::datasource::PatternAnalysis>,
...
mod tests {
    ...
    fn test_format_compliance_report_summary() {
        use scope::compliance::risk::{RiskAssessment, RiskCategory, RiskFactor, RiskLevel};
    ...
    fn test_format_compliance_report_with_pattern_analysis() {
        use scope::compliance::datasource::PatternAnalysis;
        use scope::compliance::risk::{RiskAssessment, RiskCategory, RiskFactor, RiskLevel};
        let assessment = RiskAssessment {
