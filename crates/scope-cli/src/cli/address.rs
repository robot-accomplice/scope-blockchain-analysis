...
pub async fn run(
    ...
    if args.chain == "ethereum"
        && let Some(inferred) = scope::chains::infer_chain_from_address(&args.address)
        && inferred != "ethereum"
    {
    ...
    match chain {
        ...
        "ethereum" | "polygon" | "arbitrum" | "optimism" | "base" | "bsc" | "aegis" => {
            if !address.starts_with("0x") {
                return Err(scope::error::ScopeError::InvalidAddress(format!(
...
mod tests {
    ...
    impl ChainClient for MockClient {
        ...
        async fn get_balance(&self, _addr: &str) -> scope::error::Result<ChainBalance> {
            Ok(ChainBalance {
        ...
        async fn get_transaction(&self, _h: &str) -> scope::error::Result<ChainTransaction> {
            Err(scope::error::ScopeError::NotFound("mock".into()))
        }
        ...
        async fn get_code(&self, _addr: &str) -> scope::error::Result<String> {
            Ok("0x".into())
        }
    }
    ...
    impl ChainClient for FailTxMockClient {
        ...
        async fn get_transactions(
            ...
            Err(scope::error::ScopeError::Chain("tx fetch failed".into()))
        }
        async fn get_block_number(&self) -> scope::error::Result<u64> {
            Ok(12345678)
        }
    ...
    impl ChainClient for FailTokenBalancesMockClient {
        ...
        async fn get_token_balances(
            ...
            Err(scope::error::ScopeError::Chain(
    ...
    async fn test_run_with_tokens() {
        let config = Config::default();
        let mut factory = mock_factory();
        factory.mock_client.token_balances = vec![scope::chains::TokenBalance {
            token: scope::chains::Token {
                contract_address: "0xusdc".to_string(),
                symbol: "USDC".to_string(),
                name: "USD Coin".to_string(),
                decimals: 6,
