...
pub async fn run(
    ...
    let mut rl = DefaultEditor::new().map_err(|e| {
        scope::error::ScopeError::Chain(format!("Failed to initialize readline: {}", e))
    })?;
    ...
    match command.as_str() {
        ...
        "address" | "addr" => {
            ...
            let effective_chain = if let Some(chain) = chain_override {
                ...
                if let Some(inferred) = scope::chains::infer_chain_from_address(&addr) {
        ...
        "tx" | "transaction" => {
            ...
            let effective_chain = if let Some(chain) = chain_override {
                ...
                if let Some(inferred) = scope::chains::infer_chain_from_hash(&hash) {
                    eprintln!("  Chain: {} (auto-detected)", inferred);
                    inferred.to_string()
                } else {
                    "ethereum".to_string()
        ...
        "crawl" | "token" => {
            ...
            let effective_chain = if let Some(chain) = chain_override {
                ...
                if let Some(inferred) = scope::chains::infer_chain_from_address(&token) {
                    eprintln!("  Chain: {} (auto-detected)", inferred);
                    inferred.to_string()
                } else {
                    "ethereum".to_string()
            ...
            AddressBookArgs {
                command: AddressBookCommands::Add(AddArgs {
                    chain: if context.is_auto_chain() {
                        scope::chains::infer_chain_from_address(&address)
                            .unwrap_or("ethereum")
                            .to_string()
                    } else {
                        context.chain.clone()
...
mod tests {
    ...
    fn test_factory() -> scope::chains::DefaultClientFactory {
