...
use scope::display::{charts, report};
use scope::error::{Result, ScopeError};
use scope::market::{ExchangeClient, VenueRegistry};
...
fn select_token_impl<'a>(
    ...
    for (i, token) in results.iter().enumerate() {
        ...
        let liquidity = scope::display::format_large_number(token.liquidity_usd);
...
async fn fetch_analytics_with_dex(
    ...
    args: &CrawlArgs,
    clients: &dyn ChainClientFactory,
    dex_data: scope::chains::dex::DexTokenData,
) -> Result<TokenAnalytics> {
    ...
    let socials: Vec<scope::chains::TokenSocial> = dex_data
        .socials
        .iter()
        .map(|s| scope::chains::TokenSocial {
...
mod tests {
    ...
    fn test_format_large_number() {
        assert_eq!(scope::display::format_large_number(500.0), "500.00");
        assert_eq!(scope::display::format_large_number(1500.0), "1.50K");
        assert_eq!(scope::display::format_large_number(1500000.0), "1.50M");
        assert_eq!(scope::display::format_large_number(1500000000.0), "1.50B");
    }
    ...
    fn test_format_large_number_zero() {
        assert_eq!(scope::display::format_large_number(0.0), "0.00");
    }

    #[test]
    fn test_format_large_number_small() {
        assert_eq!(scope::display::format_large_number(0.12), "0.12");
    }

    #[test]
    fn test_format_large_number_boundary_k() {
        assert_eq!(scope::display::format_large_number(999.99), "999.99");
        assert_eq!(scope::display::format_large_number(1000.0), "1.00K");
    }

    #[test]
    fn test_format_large_number_boundary_m() {
        assert_eq!(scope::display::format_large_number(999_999.0), "1000.00K");
        assert_eq!(scope::display::format_large_number(1_000_000.0), "1.00M");
    }

    #[test]
    fn test_format_large_number_boundary_b() {
        assert_eq!(
            scope::display::format_large_number(999_999_999.0),
            "1000.00M"
        );
        assert_eq!(
            scope::display::format_large_number(1_000_000_000.0),
            "1.00B"
        );
    }
    ...
    fn test_format_large_number_very_large() {
        let result = scope::display::format_large_number(1_500_000_000_000.0);
    ...
    use scope::chains::mocks::{MockClientFactory, MockDexSource};
    ...
    fn test_format_large_number_negative() {
        let result = scope::display::format_large_number(-1_000_000.0);
