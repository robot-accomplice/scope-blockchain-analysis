...
async fn lookup_openchain(
    ...
    let response = http_client
        .send(crate::http::Request::get(&url))
        .await
        .ok()?;
    let body: OpenChainResponse = response.json().ok()?;
...
async fn lookup_4byte(selector: &str, http_client: &dyn crate::http::HttpClient) -> Option<String> {
    ...
    let response = http_client
        ...
        .await
        .ok()?;
    let body: FourByteResponse = response.json().ok()?;
...
mod tests {
    ...
    fn test_native_transfer_decoding() {
        ...
        let result = rt.block_on(async {
            let client = crate::http::NativeHttpClient::new().unwrap();
