...
use scope_cli::cli::insights::{self, InsightsArgs};
...
pub async fn handle(
    ...
    match &target {
        insights::InferredTarget::Address { chain } => {
            let addr_args = scope_cli::cli::address::AddressArgs {
            ...
            let client: Box<dyn scope::chains::ChainClient> =
        ...
        insights::InferredTarget::Transaction { chain } => {
            match scope_cli::cli::tx::fetch_transaction_report(
                &target_str,
                chain,
                args.decode,
                args.trace,
        ...
        insights::InferredTarget::Token { chain } => {
            match scope_cli::cli::crawl::fetch_analytics_for_input(
                &target_str,
                chain,
                scope_cli::cli::crawl::Period::Hour24,
                10,
                &state.factory,
                None,
            )
